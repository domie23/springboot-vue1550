...
<div class="section1">
            ...
            <div class="layui-form-item">
                                    ...
                                    <input type="text" name="title" required lay-verify="required" autocomplete="off"
                                           class="layui-input">
                                                            ...
                                                            <div class="layui-input-block block">
                                                                        <select name="user_gender" lay-filter="user_gender" id="user_gender">
                                        <option value=""></option>
                                    </select>
                                                                    </div>
    ...
    <div class="table">
        <table class="layui-hide" id="ordinary_users" lay-filter="ordinary_users"></table>
...
<script src="../../js/axios.min.js"></script>
    ...
    layui.use(['element', 'layer', 'util'], function () {
        ...
        let url = BaseUrl + '/api/ordinary_users/get_list?like=0'
        ...
        if (user_group == "管理员" || $check_action('/ordinary_users/table', 'del') || $check_action('/ordinary_users/view', 'del')) {
            deletes.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/ordinary_users/table', 'add') || $check_action('/ordinary_users/view', 'add')) {
            add.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/ordinary_users/view', 'get') || $check_action('/ordinary_users/view', 'set')) {
            detail_flag = true;
        }
        ...
        function getpath() {
            ...
            for (var i = 0; i < list_data.length; i++) {
                var o = list_data[i];
                if (o.path === "/ordinary_users/table") {
                    console.log(o.path);
                    path1 = o.path
                        $get_power(o.path)
                }
 ...
 function open_tip(arr) {
        ...
        , cols: [[
            ...
            field: 'user_id', width: 200, title: '用户名', templet: function (d) {
                ...
                field: 'user_name', width: 180, title: '用户姓名', sort: true
                                ...
                                , {
                field: 'user_gender', width: 180, title: '用户性别', sort: true
            ...
            if (res.count == null) { // 无数据时直接返回了
                return;
                        ...
                        $("[data-field='user_name']").css('display', 'none');
                                        ...
                                        if (user_group === '管理员' || $check_field('get', 'user_gender', path1)) {
                        // console.log("显示")
                    } else {
                        $("[data-field='user_gender']").css('display', 'none');
                    }
    ...
    ,
        ...
        open_tip(res.result.list);
            ...
            if (user_group == "管理员" || $check_pay('/ordinary_users/table')) {
                for (var i = 0; i < res.result.list.length; i++) {
                    res.result.list[i].check_pay = true
                }
            }
            if (user_group == "管理员" || $check_action('/ordinary_users/view', 'set') || $check_action('/ordinary_users/view', 'get')) {
                for (var i = 0; i < res.result.list.length; i++) {
    ...
    ,
        ...
        table.on('tool(ordinary_users)', function (obj) {
            var data = obj.data;
        if (obj.event === 'detail')
        {
        ...
        //下拉框
                                            ...
                                            // 用户性别选项列表
                let user_gender_data = ['男','女'];
                                async function user_gender() {
                    var user_gender = document.querySelector("#user_gender")
                    var op1 = document.createElement("option");
                        user_gender.appendChild(op1)
                    // 收集数据 长度
                    var count
                    // 收集数据 数组
                    var arr = []
                    count = user_gender_data.length
                    arr = user_gender_data
                    for (var i = 0; i < arr.length; i++) {
                        ...
                        op.value = arr[i]
                        // 新增/Add节点
                            user_gender.appendChild(op)
                        layui.form.render("select");
                    }
                }

                layui.form.on('select(user_gender)', function (data) {
                    request.user_gender = data.elem[data.elem.selectedIndex].text;
                })
                    user_gender()
        ...
        let resetSelect = []
        //下拉框重置/Reset参数
        fun('ordinary_users', BaseUrl + '/api/ordinary_users/del', 'ordinary_users_id', request, resetName, resetSelect                        , undefined, undefined,true
        )
