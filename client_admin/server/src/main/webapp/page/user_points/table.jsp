...
<div class="section1">
    ...
    <div class="manu" style="padding: 15px;">
        <form class="layui-form" action="">
            <div class="layui-form-item">
                                    ...
                                    <select name="ordinary_users" lay-filter="ordinary_users" id="ordinary_users">
                                        <option value=""></option>
                                    </select>
                                </div>
                                                                </div>
    ...
    <div class="table">
        <table class="layui-hide" id="user_points" lay-filter="user_points"></table>
...
<script src="../../js/axios.min.js"></script>
    ...
    layui.use(['element', 'layer', 'util'], function () {
        ...
        let url = BaseUrl + '/api/user_points/get_list?like=0'
        ...
        if (user_group == "管理员" || $check_action('/user_points/table', 'del') || $check_action('/user_points/view', 'del')) {
            deletes.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/user_points/table', 'add') || $check_action('/user_points/view', 'add')) {
            add.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/user_points/view', 'get') || $check_action('/user_points/view', 'set')) {
            detail_flag = true;
        }
 ...
 function open_tip(arr) {
        ...
        , cols: [[
                ...
                field: 'ordinary_users', width: 180, title: '普通用户', sort: true
                            ...
                            function (d) {
                                ...
                                , {
                field: 'total_points', width: 180, title: '积分总数', sort: true
            ...
            if (res.count == null) { // 无数据时直接返回了
                return;
                        ...
                        $("[data-field='ordinary_users']").css('display', 'none');
                                        ...
                                        if (user_group === '管理员' || $check_field('get', 'total_points', path1)) {
                        // console.log("显示")
                    } else {
                        $("[data-field='total_points']").css('display', 'none');
                    }
    ...
    ,
        ...
        open_tip(res.result.list);
            ...
            if (user_group == "管理员" || $check_pay('/user_points/table')) {
                for (var i = 0; i < res.result.list.length; i++) {
                    res.result.list[i].check_pay = true
                }
            }
            if (user_group == "管理员" || $check_action('/user_points/view', 'set') || $check_action('/user_points/view', 'get')) {
                for (var i = 0; i < res.result.list.length; i++) {
    ...
    ,
        ...
        table.on('tool(user_points)', function (obj) {
            var data = obj.data;
        if (obj.event === 'detail')
        {

                            window.location.href = "./view_add.jsp?" + data.user_points_id;
                    }
        ...
        let resetSelect = []
        //下拉框重置/Reset参数
        fun('user_points', BaseUrl + '/api/user_points/del', 'user_points_id', request, resetName, resetSelect        )
