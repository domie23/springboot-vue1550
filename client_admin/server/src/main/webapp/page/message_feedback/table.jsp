...
<div class="section1">
    ...
    <div class="table">
        <table class="layui-hide" id="message_feedback" lay-filter="message_feedback"></table>
...
<script src="../../js/axios.min.js"></script>
    ...
    layui.use(['element', 'layer', 'util'], function () {
        ...
        let url = BaseUrl + '/api/message_feedback/get_list?like=0'
        ...
        if (user_group == "管理员" || $check_action('/message_feedback/table', 'del') || $check_action('/message_feedback/view', 'del')) {
            deletes.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/message_feedback/table', 'add') || $check_action('/message_feedback/view', 'add')) {
            add.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/message_feedback/view', 'get') || $check_action('/message_feedback/view', 'set')) {
            detail_flag = true;
        }
 ...
 function open_tip(arr) {
    ...
    table.render({
        ...
        , cols: [[
                ...
                field: 'user_name', width: 180, title: '用户姓名', sort: true
                                ...
                                , {
                field: 'message_content', width: 180, title: '留言内容', sort: true
                                                                                
            }
                                , {
                field: 'message_date', width: 180, title: '留言日期', sort: true
                                                    , templet: "<div>{{layui.util.toDateString(d.message_date, 'yyyy-MM-dd')}}</div>"
            ...
            if (res.count == null) { // 无数据时直接返回了
                return;
                        ...
                        $("[data-field='user_name']").css('display', 'none');
                                        ...
                                        if (user_group === '管理员' || $check_field('get', 'message_content', path1)) {
                        // console.log("显示")
                    } else {
                        $("[data-field='message_content']").css('display', 'none');
                    }
                                        if (user_group === '管理员' || $check_field('get', 'message_date', path1)) {
                        // console.log("显示")
                    } else {
                        $("[data-field='message_date']").css('display', 'none');
                    }
    ...
    ,
        ...
        open_tip(res.result.list);
            ...
            if (user_group == "管理员" || $check_pay('/message_feedback/table')) {
                for (var i = 0; i < res.result.list.length; i++) {
                    res.result.list[i].check_pay = true
                }
            }
            if (user_group == "管理员" || $check_action('/message_feedback/view', 'set') || $check_action('/message_feedback/view', 'get')) {
                for (var i = 0; i < res.result.list.length; i++) {
    ...
    ,
        ...
        table.on('tool(message_feedback)', function (obj) {
            var data = obj.data;
        if (obj.event === 'detail')
        {

                            window.location.href = "./view_add.jsp?" + data.message_feedback_id;
                    }
        ...
        let resetSelect = []
        //下拉框重置/Reset参数
        fun('message_feedback', BaseUrl + '/api/message_feedback/del', 'message_feedback_id', request, resetName, resetSelect        )
