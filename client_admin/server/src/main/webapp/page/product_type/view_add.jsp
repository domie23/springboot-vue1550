...
<script>
    var BaseUrl = baseUrl()
    let product_type_id = location.search.substring(1)
    layui.use(['upload', 'element', 'layer', 'laydate', 'layedit'], function () {
        ...
        // 提交按钮校验权限
        // if (   user_group == "管理员" ||$check_action('/product_type/view', 'add') || $check_action('/product_type/view', 'set')) {
        //    submit.style.display = "block"
        // }
        // style="display: none"

        
            let field = "product_type_id";
            let url_add = "product_type";
            let url_set = "product_type";
            let url_get_obj = "product_type";
            let url_upload = "product_type"
            let query = {
                "product_type_id": 0,
            }
            ...
            layui.layedit.set({
      uploadImage: {
        url: BaseUrl + '/api/product_type/upload?' //接口url
        , type: 'post' //默认post
      }
    });
            ...
            function $check_field(action, field) {
                var o = $get_power("/product_type/view");
                var auth;
                if (o && o[action] != 0 && o[action] != false) {
                    auth = o["field_" + action];
                }
            ...
            if (product_type_id !== '') {
                async function axios_get_3() {
                    const {data: rese} = await axios.get(BaseUrl + '/api/product_type/get_obj', {
                        params: {
                    ...
                    // Array.prototype.slice.call(document.getElementsByTagName('textarea')).map(i => i.disabled = true)
                            ...
                            //文本
                                ...
                                if (user_group === '管理员' || (form_data2['product_type_id'] && $check_field('set', 'product_type')) || (!form_data2['product_type_id'] && $check_field('add', 'product_type'))) {
                            }else {
            ...
            submit.onclick = async function () {
                ...
                if (product_type_id == '') {
                                        console.log("新增/Add")
                    const {data: res} = await axios.post(BaseUrl + '/api/product_type/add?',
                            form_data2, {
                                headers: {
                                    'x-auth-token': token,
              ...
              layer.msg(res.error.message);
                    ...
                    const {data: res} = await axios.post(BaseUrl + '/api/product_type/set?product_type_id=' + product_type_id,
                            form_data2, {
                                headers: {
                                    'x-auth-token': token,
                                    'Content-Type': 'application/json'
