...
<div class="section1">
    ...
    <div class="table">
        <table class="layui-hide" id="product_information" lay-filter="product_information"></table>
...
<script src="../../js/axios.min.js"></script>
    ...
    layui.use(['element', 'layer', 'util'], function () {
        ...
        let url = BaseUrl + '/api/product_information/get_list?like=0'
        ...
        if (user_group == "管理员" || $check_action('/product_information/table', 'del') || $check_action('/product_information/view', 'del')) {
            deletes.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/product_information/table', 'add') || $check_action('/product_information/view', 'add')) {
            add.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/product_information/view', 'get') || $check_action('/product_information/view', 'set')) {
            detail_flag = true;
        }
 ...
 function open_tip(arr) {
    ...
    table.render({
        ...
        , cols: [[
            ...
            templet: "<div><img src={{fullUrl(BaseUrl,d.cart_img)}}></div>"
                            ...
                            , {field: 'cart_title', width: 180, title: '标题', sort: true}
            
            , {field: 'cart_price', width: 180, title: '卖价', sort: true}
            , {field: 'cart_price_ago', width: 180, title: '原价', sort: true}
            , {field: 'cart_inventory', width: 180, title: '商品库存', sort: true}
            , {field: 'cart_type', width: 180, title: '产品分类', sort: true}
            , {field: 'cart_description', width: 180, title: '描述', sort: true}
        
                    , {
                field: 'commodity_specifications', width: 180, title: '商品规格', sort: true
    ...
    ,
            ...
            if (res.count == null) { // 无数据时直接返回了
                return;
                                    ...
                                    if (user_group === '管理员' || $check_field('get', 'commodity_specifications', path1)) {
                        // console.log("显示")
                    } else {
                        $("[data-field='commodity_specifications']").css('display', 'none');
                    }
    ...
    ,
        ...
        open_tip(res.result.list);
            ...
            if (user_group == "管理员" || $check_pay('/product_information/table')) {
                for (var i = 0; i < res.result.list.length; i++) {
                    res.result.list[i].check_pay = true
                }
            }
            if (user_group == "管理员" || $check_action('/product_information/view', 'set') || $check_action('/product_information/view', 'get')) {
                for (var i = 0; i < res.result.list.length; i++) {
    ...
    ,
        ...
        table.on('tool(product_information)', function (obj) {
            var data = obj.data;
        if (obj.event === 'detail')
        {

                            window.location.href = "./view_add.jsp?" + data.product_information_id;
                    }
        ...
        let resetSelect = []
        //下拉框重置/Reset参数
        fun('product_information', BaseUrl + '/api/product_information/del', 'product_information_id', request, resetName, resetSelect                , 'product_information_id', 'product_information'
                )
