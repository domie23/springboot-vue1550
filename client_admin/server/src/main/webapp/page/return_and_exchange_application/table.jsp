...
<div class="section1">
    ...
    <div class="table">
        <table class="layui-hide" id="return_and_exchange_application" lay-filter="return_and_exchange_application"></table>
...
<script src="../../js/axios.min.js"></script>
    ...
    layui.use(['element', 'layer', 'util'], function () {
        ...
        let url = BaseUrl + '/api/return_and_exchange_application/get_list?like=0'
        ...
        if (user_group == "管理员" || $check_action('/return_and_exchange_application/table', 'del') || $check_action('/return_and_exchange_application/view', 'del')) {
        ...
        if (user_group == "管理员" || $check_action('/return_and_exchange_application/view', 'get') || $check_action('/return_and_exchange_application/view', 'set')) {
            detail_flag = true;
        }
 ...
 function open_tip(arr) {
    ...
    table.render({
        elem: '#return_and_exchange_application'
        , toolbar: true
        , url: url
        , headers: {
            'x-auth-token': token,
        ...
        , cols: [[
                ...
                field: 'product_name', width: 180, title: '商品名称', sort: true
                                ...
                                , {
                field: 'operation_date', width: 180, title: '操作日期', sort: true
                                                    , templet: "<div>{{layui.util.toDateString(d.operation_date, 'yyyy-MM-dd')}}</div>"
                                                                
            }
                                , {
                field: 'return_image', width: 180, title: '退货图片', sort: true
                                    , templet: function (d) {
                        return "<div><img src=" + fullUrl(BaseUrl,d.return_image) + "></div>"
                    }
                                                                                
            }
                                , {
                field: 'application_type', width: 180, title: '申请类型', sort: true
                                                                                
            }
                                , {
                field: 'explain_the_reason', width: 180, title: '说明原因', sort: true
            ...
            if (res.count == null) { // 无数据时直接返回了
                return;
                        ...
                        $("[data-field='product_name']").css('display', 'none');
                                        ...
                                        if (user_group === '管理员' || $check_field('get', 'operation_date', path1)) {
                        // console.log("显示")
                    } else {
                        $("[data-field='operation_date']").css('display', 'none');
                    }
                                        if (user_group === '管理员' || $check_field('get', 'return_image', path1)) {
                        // console.log("显示")
                    } else {
                        $("[data-field='return_image']").css('display', 'none');
                    }
                                        if (user_group === '管理员' || $check_field('get', 'application_type', path1)) {
                        // console.log("显示")
                    } else {
                        $("[data-field='application_type']").css('display', 'none');
                    }
                                        if (user_group === '管理员' || $check_field('get', 'explain_the_reason', path1)) {
                        // console.log("显示")
                    } else {
                        $("[data-field='explain_the_reason']").css('display', 'none');
                    }
    ...
    ,
        ...
        open_tip(res.result.list);
            ...
            if (user_group == "管理员" || $check_pay('/return_and_exchange_application/table')) {
                for (var i = 0; i < res.result.list.length; i++) {
                    res.result.list[i].check_pay = true
                }
            }
            if (user_group == "管理员" || $check_action('/return_and_exchange_application/view', 'set') || $check_action('/return_and_exchange_application/view', 'get')) {
                for (var i = 0; i < res.result.list.length; i++) {
    ...
    ,
        ...
        table.on('tool(return_and_exchange_application)', function (obj) {
            var data = obj.data;
        if (obj.event === 'detail')
        {

                            window.location.href = "./view_add.jsp?" + data.return_and_exchange_application_id;
                    }
        ...
        //下拉框
                ...
                layui.form.on('select(application_type)', function (data) {
                    request.application_type = data.elem[data.elem.selectedIndex].text;
                })
                    application_type()
        ...
        let resetSelect = []
        //下拉框重置/Reset参数
        fun('return_and_exchange_application', BaseUrl + '/api/return_and_exchange_application/del', 'return_and_exchange_application_id', request, resetName, resetSelect        )
