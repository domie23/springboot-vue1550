...
<div class="section1">
    ...
    <div class="table">
        <table class="layui-hide" id="sales_information" lay-filter="sales_information"></table>
...
<script src="../../js/axios.min.js"></script>
    ...
    layui.use(['element', 'layer', 'util'], function () {
        ...
        let url = BaseUrl + '/api/sales_information/get_list?like=0'
        ...
        if (user_group == "管理员" || $check_action('/sales_information/table', 'del') || $check_action('/sales_information/view', 'del')) {
            deletes.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/sales_information/table', 'add') || $check_action('/sales_information/view', 'add')) {
            add.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/sales_information/view', 'get') || $check_action('/sales_information/view', 'set')) {
            detail_flag = true;
        }
 ...
 function open_tip(arr) {
    ...
    table.render({
        ...
        , cols: [[
                ...
                field: 'product_type', width: 180, title: '商品类型', sort: true
                                ...
                                , {
                field: 'sales_date', width: 180, title: '销售日期', sort: true
                                                    , templet: "<div>{{layui.util.toDateString(d.sales_date, 'yyyy-MM-dd')}}</div>"
                                                                
            }
                                , {
                field: 'sales_amount', width: 180, title: '销售金额', sort: true
            ...
            if (res.count == null) { // 无数据时直接返回了
                return;
                        ...
                        $("[data-field='product_type']").css('display', 'none');
                                        ...
                                        if (user_group === '管理员' || $check_field('get', 'sales_date', path1)) {
                        // console.log("显示")
                    } else {
                        $("[data-field='sales_date']").css('display', 'none');
                    }
                                        if (user_group === '管理员' || $check_field('get', 'sales_amount', path1)) {
                        // console.log("显示")
                    } else {
                        $("[data-field='sales_amount']").css('display', 'none');
                    }
    ...
    ,
        ...
        open_tip(res.result.list);
            ...
            if (user_group == "管理员" || $check_pay('/sales_information/table')) {
                for (var i = 0; i < res.result.list.length; i++) {
                    res.result.list[i].check_pay = true
                }
            }
            if (user_group == "管理员" || $check_action('/sales_information/view', 'set') || $check_action('/sales_information/view', 'get')) {
                for (var i = 0; i < res.result.list.length; i++) {
    ...
    ,
        ...
        table.on('tool(sales_information)', function (obj) {
            var data = obj.data;
        if (obj.event === 'detail')
        {

                            window.location.href = "./view_add.jsp?" + data.sales_information_id;
                    }
        ...
        let resetSelect = []
        //下拉框重置/Reset参数
        fun('sales_information', BaseUrl + '/api/sales_information/del', 'sales_information_id', request, resetName, resetSelect        )
