...
<div class="section1">
    ...
    <div class="table">
        <table class="layui-hide" id="redemption_records" lay-filter="redemption_records"></table>
...
<script src="../../js/axios.min.js"></script>
    ...
    layui.use(['element', 'layer', 'util'], function () {
        ...
        let url = BaseUrl + '/api/redemption_records/get_list?like=0'
        ...
        if (user_group == "管理员" || $check_action('/redemption_records/table', 'del') || $check_action('/redemption_records/view', 'del')) {
            deletes.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/redemption_records/table', 'add') || $check_action('/redemption_records/view', 'add')) {
            add.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/redemption_records/view', 'get') || $check_action('/redemption_records/view', 'set')) {
            detail_flag = true;
        }
 ...
 function open_tip(arr) {
    ...
    table.render({
        ...
        , cols: [[
                    ...
                    , {
                field: 'exchange_number', width: 180, title: '兑换号', sort: true
                ...
                field: 'required_points', width: 180, title: '所需积分', sort: true
                                ...
                                , {
                field: 'redemption_date', width: 180, title: '兑换日期', sort: true
                                                    , templet: "<div>{{layui.util.toDateString(d.redemption_date, 'yyyy-MM-dd')}}</div>"
    ...
    ,
            ...
            if (res.count == null) { // 无数据时直接返回了
                return;
                                    ...
                                    if (user_group === '管理员' || $check_field('get', 'exchange_number', path1)) {
                        // console.log("显示")
                    } else {
                        $("[data-field='exchange_number']").css('display', 'none');
                    }
                        ...
                        $("[data-field='required_points']").css('display', 'none');
                                        ...
                                        if (user_group === '管理员' || $check_field('get', 'redemption_date', path1)) {
                        // console.log("显示")
                    } else {
                        $("[data-field='redemption_date']").css('display', 'none');
                    }
    ...
    ,
        ...
        open_tip(res.result.list);
            ...
            if (user_group == "管理员" || $check_pay('/redemption_records/table')) {
                for (var i = 0; i < res.result.list.length; i++) {
                    res.result.list[i].check_pay = true
                }
            }
            if (user_group == "管理员" || $check_action('/redemption_records/view', 'set') || $check_action('/redemption_records/view', 'get')) {
                for (var i = 0; i < res.result.list.length; i++) {
    ...
    ,
        ...
        table.on('tool(redemption_records)', function (obj) {
            var data = obj.data;
        if (obj.event === 'detail')
        {

                            window.location.href = "./view_add.jsp?" + data.redemption_records_id;
                    }
        ...
        let resetSelect = []
        //下拉框重置/Reset参数
        fun('redemption_records', BaseUrl + '/api/redemption_records/del', 'redemption_records_id', request, resetName, resetSelect        )
